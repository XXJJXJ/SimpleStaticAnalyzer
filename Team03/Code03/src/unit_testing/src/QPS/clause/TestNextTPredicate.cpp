...
TEST_CASE("Test nextT table retrieval", "[NextTPredicate]") {
    ...
    qm.addFakeNextT(stmt1, stmt2); // print follows print
    qm.addFakeNextT(stmt2, stmt3); // print follows read
    qm.addFakeNextT(stmt3, stmt4); // read follows read
    qm.addFakeNextT(stmt4, stmt5); // read follows read

    SECTION("Using statement numbers") {
        // All Failing here

        SECTION("NextT(1, 2) is true") {
            NextTPredicate followsPred(1, 2);
        ...
        SECTION("NextT(5, 4) is false") {
            NextTPredicate followsPred(5, 4);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->isBoolean());
            auto boolTable = dynamic_pointer_cast<BooleanTable>(table);
            REQUIRE(!boolTable->isTrue());
        ...
        SECTION("NextT(4, 3) is false") {
            NextTPredicate followsPred(4, 3);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->isBoolean());
            auto boolTable = dynamic_pointer_cast<BooleanTable>(table);
            REQUIRE(!boolTable->isTrue());
        ...
        SECTION("NextT(3, 2) is false") {
            NextTPredicate followsPred(3, 2);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->isBoolean());
            auto boolTable = dynamic_pointer_cast<BooleanTable>(table);
            REQUIRE(!boolTable->isTrue());
        ...
        SECTION("NextT(2, 1) is false") {
            NextTPredicate followsPred(2, 1);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->isBoolean());
            auto boolTable = dynamic_pointer_cast<BooleanTable>(table);
            REQUIRE(!boolTable->isTrue());
    ...
    SECTION("Using synonyms only") {
        SECTION("Using stmt synonym type - gets all") {
            ...
            NextTPredicate followsPred(stmtSyn, stmtSyn2);
            auto table = followsPred.getResultTable(qm);
    ...
    SECTION("Using mix of synonyms and statement numbers") {
        Synonym stmtSyn(EntityType::Stmt, "s1");
        SECTION("NextT(1, s1) -- gets 1") {
            NextTPredicate followsPred(1, stmtSyn);
            auto table = followsPred.getResultTable(qm);
        ...
        SECTION("NextT(s1, 1) -- gets 0") {
            NextTPredicate followsPred(stmtSyn, 1);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->getColumnCount() == 1);
            REQUIRE(table->getRows().size() == 0);
        }

        SECTION("NextT(s1, 3) -- gets 1") {
            NextTPredicate followsPred(stmtSyn, 3);
            auto table = followsPred.getResultTable(qm);
            REQUIRE(table->getColumnCount() == 1);
            REQUIRE(table->getRows().size() == 1);
            REQUIRE(table->getRows()[0].getValues()[0]->getName() == "2");
