...
TEST_CASE(
    "QueryValidator::splitTokens should correctly split query into lists of tokens, where each list is a part of the query") {
    std::vector<std::string> testQuery =
        {"variable", "a", ";", "Select", "a", "such", "that", "Follows", "(", "a", ",", "_", ")", "with", "a.stmt#",
         "=", "8", "pattern", "a", "(", "_", ",", "_", ")"};

    std::vector<std::vector<std::vector<std::string>>> result = QueryValidator::splitTokens(testQuery);

    std::vector<std::vector<std::vector<std::string>>> expectedResult = {{{"variable", "a", ";"}}, {{"Select", "a"}},
                                                                         {{"such", "that", "Follows", "(", "a", ",",
                                                                           "_", ")"}, {"with", "a.stmt#", "=", "8"},
                                                                          {"pattern", "a", "(", "_", ",", "_", ")"}}};
...
TEST_CASE("QueryValidator::splitTokens should support synonyms with names that are same as keywords") {
    std::vector<std::string>
        testQuery = {"variable", "Select", ",", "assign", ";", "assign", "variable", ";", "Select", "Select"};

    std::vector<std::vector<std::vector<std::string>>> result = QueryValidator::splitTokens(testQuery);

    std::vector<std::vector<std::vector<std::string>>> expectedResult =
        {{{"variable", "Select", ",", "assign", ";"}, {"assign", "variable", ";"}}, {{"Select", "Select"}}, {}};

    REQUIRE(result == expectedResult);
}

TEST_CASE("QueryValidator::splitTokens should throw error if clauses are in the wrong order") {
    std::vector<std::string> testQuery1 =
        {"variable", "a", ",", "b", ";", "assign", "c", ";", "Select", "a", "such", "that", "Follows", "(", "a", ",",
         "_", "_", ")", "pattern", "a", "(", "_", "_", ",", "_", "_", ")"};
    std::vector<std::string> testQuery2 =
        {"variable", "a", ",", "b", ";", "Select", "a", "assign", "c", ";", "such", "that", "Follows", "(", "a", ",",
         "_", "_", ")", "pattern", "a", "(", "_", "_", ",", "_", "_", ")"};  // Declaration after Select
    std::vector<std::string> testQuery3 =
        {"variable", "a", ",", "b", ";", "such", "that", "Follows", "(", "a", ",", "_", "_", ")", "assign", "c", ";",
         "Select", "a", "pattern", "a", "(", "_", "_", ",", "_", "_", ")"};  // Clause before Declaration
    std::vector<std::string> testQuery4 =
        {"variable", "a", ",", "b", ";", "assign", "c", ";", "such", "that", "Follows", "(", "a", ",", "_", "_", ")",
         "pattern", "a", "(", "_", "_", ",", "_", "_", ")", "Select", "a"};  // Select after clause
...
TEST_CASE("Test QueryValidator::validateDeclaration") {
    ...
    SECTION("Invalid declarations") {
        ...
        std::vector<std::string>
            tokens4 = {"variable", "123invalidName", ",", "b", ",", "c", ";"}; // Invalid synonym name
...
TEST_CASE("Test QueryValidator::validateSelection") {
    SECTION("Valid selections") {
        ...
        std::vector<std::string>
            tokens5 = {"Select", "<", "a", ",", "a.procName", ",", "a.varName", ",", "a.value", ",", "a.stmt#", ">"};
...
TEST_CASE("Test QueryValidator::validatePredicate") {
    ...
    SECTION("Invalid predicates") {
        std::vector<std::string>
            tokens1 = {"such", "that", "pattern", "a", "(", "_", ",", "_", ")"}; // such that pattern
        ...
        REQUIRE_THROWS_AS(QueryValidator::validatePredicate(tokens3, patternClause),
                          SyntaxErrorException); // Pattern pred and such that pred
        REQUIRE_THROWS_AS(QueryValidator::validatePredicate(tokens4, suchThatClause),
                          SyntaxErrorException); // Such that pred and pattern pred
        REQUIRE_THROWS_AS(QueryValidator::validatePredicate(tokens5, noPrevClause), SyntaxErrorException);
        REQUIRE_THROWS_AS(QueryValidator::validatePredicate(tokens6, patternClause),
                          SyntaxErrorException); // Pattern pred and with pred
    }
}
...
TEST_CASE("Test QueryValidator::validateStatementStatementPredicate") {
    ...
    SECTION("Invalid StatementStatementPredicates") {
        std::vector<std::string>
            tokens1 = {"Follows", "(", "a", ",", "b", ",", "c", ")"}; // Invalid number of arguments
...
TEST_CASE("Test QueryValidator::validateStmtEntEntityPredicate") {
    SECTION("Valid StatementEntityPredicates") {
        ...
        std::vector<std::string> tokens3 = {"Modifies", "(", "\"validProcName\"", ",", "\"validVarName\"",
                                            ")"}; // LHS valid procedure name, RHS valid variable name
        std::vector<std::string> tokens4 = {"Modifies", "(", "_", ",", "b",
                                            ")"}; // LHS wildcard, syntactically valid so will not get caught here, but will be caught later on as a semantic error
    ...
    SECTION("Invalid StatementEntityPredicates") {
        ...
        std::vector<std::string>
            tokens5 = {"Modifies", "(", "a", ",", "\"123invalidString\"", ")"}; // RHS invalid string
...
TEST_CASE("Test QueryValidator::validateEntityEntityPredicate") {
    SECTION("Valid EntityEntityPredicates") {
        ...
        std::vector<std::string>
            tokens3 = {"CallsT", "(", "\"validProcName\"", ",", "\"validProcName\"", ")"}; // Both valid procedure names
    ...
    SECTION("Invalid EntityEntityPredicate") {
        ...
        std::vector<std::string> tokens3 =
            {"CallsT", "(", "\"123InvalidProcName\"", ",", "\"validProcName\"", ")"}; // LHS invalid procedure name
...
TEST_CASE("Test QueryValidator::validatePatternPredicate") {
    SECTION("Valid PatternPredicates") {
        ...
        std::vector<std::string>
            tokens2 = {"a", "(", "\"validString\"", ",", "_\"x+y\"_", ")"}; // LHS valid string, RHS partial match
        std::vector<std::string>
            tokens3 = {"a", "(", "validName", ",", "\"x+y\"", ")"}; // LHS valid synonym, RHS complete match
    ...
    SECTION("Invalid PatternPredicates") {
        ...
        std::vector<std::string>
            tokens5 = {"a", "(", "_", ",", "validName", ",", "\"validString\"", ")"}; // Invalid If pattern
...
TEST_CASE("Test QueryValidator::validate") {
    SECTION("Valid query") {
        std::vector<std::string> tokens1 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "such", "that", "Follows",
             "(", "a", ",", "b", ")", "pattern", "a", "(", "_", ",", "_", ")", "with", "a.stmt#", "=", "8"};
        std::vector<std::string> tokens2 =
            {"Select", "<", "a", ",", "b", ",", "c", ">", "such", "that", "Follows", "(", "a", ",", "b", ")", "pattern",
             "a", "(", "_", ",", "_", ")"}; // No declaration, syntactically valid but semantically invalid
        std::vector<std::string> tokens3 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "such", "that", "Follows",
             "(", "a", ",", "b", ")", "and", "Modifies", "(", "a", ",", "b", ")", "and", "Uses", "(", "a", ",", "b",
             ")", "pattern", "a", "(", "_", ",", "_", ")", "and", "b", "(", "validName", ",", "_", ")", "and", "b", "(",
             "validName", ",", "_", ",", "_", ")", "with", "a.stmt#", "=", "8", "and", "b.varName", "=",
             "\"validVarName\""};
        std::vector<std::string> tokens4 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "such", "that", "Follows",
             "(", "a", ",", "b", ")", "and", "Modifies", "(", "a", ",", "b", ")", "and", "Uses", "(", "a", ",", "b",
             ")", "pattern", "a", "(", "_", ",", "_", ")", "and", "Modifies", "(", "a", ",", "_", ")", "and", "b", "(",
             "validName", ",", "_", ",", "_",
             ")"}; // Pattern and such that, syntactically valid but semantically invalid
    ...
    SECTION("Invalid query") {
        std::vector<std::string> tokens1 =
            {"invalidKeyword", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "such", "that",
             "Follows", "(", "a", ",", "b", ")", "pattern", "a", "(", "_", ",", "_", ")"}; // Invalid declaration
        std::vector<std::string> tokens2 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "123invalidName", "such", "that",
             "Follows", "(", "a", ",", "b", ")", "pattern", "a", "(", "_", ",", "_", ")"}; // Invalid selection
        std::vector<std::string> tokens3 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "<", "a", ",", "b", ",", "c", ">",
             "such", "that", "Follows", "(", "a", ",", "b", ")", "pattern", "123invalidName", "(", "_", ",", "_",
             ")"}; // Invalid predicate
        std::vector<std::string> tokens4 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "such", "that", "Follows", "(", "a", ",",
             "b", ")", "pattern", "a", "(", "_", ",", "_", ")"}; // No selection
        std::vector<std::string> tokens5 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "such", "that", "Follows",
             "(", "a", ",", "b", ")", "and", "a", "(", "_", ",", "_", ")", "and", "Uses", "(", "a", ",", "b", ")",
             "pattern", "a", "(", "_", ",", "_", ")", "and", "b", "(", "validName", ",", "_", ")", "and", "b", "(",
             "validName", ",", "_", ",", "_", ")"}; // Such that and pattern
        std::vector<std::string> tokens6 =
            {"variable", "a", ",", "b", ",", "c", ";", "assign", "a", ";", "Select", "a", "and", "Follows", "(", "a",
             ",", "b", ")", "and", "Modifies", "(", "a", ",", "b", ")", "and", "Uses", "(", "a", ",", "b", ")",
             "pattern", "a", "(", "_", ",", "_", ")", "and", "b", "(", "validName", ",", "_", ")", "and", "b", "(",
             "validName", ",", "_", ",", "_", ")"}; // Starts with and clause
