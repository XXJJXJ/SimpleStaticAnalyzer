...
TEST_CASE("Debugging whole strings") {
    QpsManager qm;
    std::string
        string1 = "assign a; variable v; constant c; Select a such that Uses (a, c) pattern a (\"8\", _) invalid words";
    std::string string2 =
        "stmt s, s1; assign a, a1; while w; if ifs; variable v, v1; procedure p, q; constant c; read re; print pn; call cl; Select a pattern a(_, _\"x+y*v \"_)";
    ...
    std::string
        string7 = "assign a; if i; while w; Select <a, i, w> pattern a(_, _) pattern i(_, _, _) pattern w(_, _)";
    ...
    std::string string13 =
        "assign a, b; Select <a, b> such that Follows(a, b) and not Modifies(a, _) pattern a(_ , _) and not b(_, _)";
    ...
    std::string
        string16 = "assign a; Select <a, a.varName> with a.stmt#=8 and a.varName=\"varName\" such that Follows(a, _)";
