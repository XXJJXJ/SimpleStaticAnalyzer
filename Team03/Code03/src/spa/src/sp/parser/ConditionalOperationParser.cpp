...
shared_ptr<Expression> ConditionalOperationParser::parseRelationalExpression() {
    ...
    relationalOperationParser->setPairOfArguments(getIsSubExpressionPointer(),
                                                  getIndexPointer(),
...
shared_ptr<Expression> ConditionalOperationParser::parseExpression(int index, TokenType tokenType) {
    setPairOfArguments(getIsSubExpressionPointer(), make_shared<int>(index), getIsProcessedTokenPointer());
...
shared_ptr<Expression> ConditionalOperationParser::parseSubExpression(
    string operation, shared_ptr<Expression> subExpression) {
    nextToken();
    if (getTokenType() == TokenType::LEFT_PARANTHESIS) {
        ...
        if (getTokenType() == TokenType::RIGHT_PARANTHESIS) {
            ...
            subExpression ? pairOfArguments = {subExpression, subExpression_} : (nextToken(), pairOfArguments =
                                                                                                  {subExpression_,
                                                                                                   subExpression});
