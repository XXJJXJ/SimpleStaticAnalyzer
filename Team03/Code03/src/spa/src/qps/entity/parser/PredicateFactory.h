...
private:
	...
	static std::variant<std::string, int, AttrRef> stringToRef(const std::string& token, const std::unordered_map<std::string, EntityType>& synonymMap);
	...
	static std::shared_ptr<Predicate> parseWithPredicate(const std::vector<std::string>& tokens, const std::unordered_map<std::string, EntityType>& synonymMap);
