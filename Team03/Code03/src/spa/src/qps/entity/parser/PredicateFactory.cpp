...
std::shared_ptr<Predicate> PredicateFactory::createPredicate(
    ...
    switch (predicateType) {
        case PredicateType::Follows: {
            FollowsPredicate
                predicate(stringToStatementRef(tokens[1], synonymMap), stringToStatementRef(tokens[2], synonymMap));
        ...
        case PredicateType::Modifies: {
            ModifiesPredicate
                predicate(stringToStatementRef(tokens[1], synonymMap), stringToEntityRef(tokens[2], synonymMap));
        ...
        case PredicateType::Calls: {
            CallsPredicate
                predicate(stringToEntityRef(tokens[1], synonymMap), stringToEntityRef(tokens[2], synonymMap));
...
std::variant<int, Synonym, std::string> PredicateFactory::stringToStatementRef(
    const std::string &token, const std::unordered_map<std::string, EntityType> &synonymMap) {
...
std::variant<Synonym, std::string> PredicateFactory::stringToEntityRef(
    const std::string &token, const std::unordered_map<std::string, EntityType> &synonymMap) {
    size_t len = token.size();
    if (len >= 2 && token[0] == '"' && token[len - 1] == '"') {
        return token.substr(1, len - 2);
...
Ref PredicateFactory::stringToRef(
    const std::string &token, const std::unordered_map<std::string, EntityType> &synonymMap) {
    size_t len = token.size();
    if (len >= 2 && token[0] == '"' && token[len - 1] == '"') {
...
shared_ptr<AttrRef> PredicateFactory::createAttrRef(
    const std::string &token, const std::unordered_map<std::string, EntityType> &synonymMap) {
...
std::shared_ptr<Predicate> PredicateFactory::parsePatternPredicate(
    const std::vector<std::string> &tokens, const std::unordered_map<std::string, EntityType> &synonymMap) {
...
std::shared_ptr<Predicate> PredicateFactory::parseWithPredicate(
    const std::vector<std::string> &tokens, const std::unordered_map<std::string, EntityType> &synonymMap) {
...
std::shared_ptr<Predicate> PredicateFactory::parseNotPredicate(
    const std::vector<std::string> &tokens, const std::unordered_map<std::string, EntityType> &synonymMap) {
